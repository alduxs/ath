...
//
$query = "SELECT * FROM elpunto_slides";
...
$query = "SELECT * FROM elpunto_slides ORDER BY sl_publicado DESC, sl_espacio ASC, sl_orden ASC, sl_titulo ASC LIMIT ?,?";
			...
			<div class="wrapper wrapper-content animated fadeInRight">
										...
										<tbody>
											...
											<?php while ($arrContenido = $rsCont->fetch(PDO::FETCH_BOTH)) { ?>
												<tr>
                                                    ...
                                                    <?php
                                                    $query2 = "SELECT * FROM elpunto_contximagenes WHERE contximg_cont_id=" . $arrContenido["sl_id"] . " AND contximg_tipo = 3";
                                                    ...
                                                    <?php if($intQtyRecords2 > 0) {?>
                                                        <td><img src="../assets/slides/<?php echo $arrCont2["contximg_imagen"]; ?>" width="300px" /></td>
                                                    <?php } else { ?>
                                                        <td>-</td>
                                                    <?php }  ?>
													<td><?php echo $arrContenido["sl_link"]; ?></td>
                                                    <td><?php echo espaciosweb($arrContenido["sl_espacio"]); ?></td>
                                                    <td><?php echo $arrContenido["sl_orden"]; ?></td>

													<td><?php if ($arrContenido["sl_publicado"] == 0) { ?><a href="#" class="btn btn-default btn-circle"><i class="fa fa-check"></i></a><?php } else { ?><a href="#" class="btn btn-info btn-circle"><i class="fa fa-check"></i></a><?php } ?></td>
													
													<td class="tooltip-demo">
														<a href="updSlides.php?seccion=slides&id=<?php echo $arrContenido["sl_id"]; ?>&intPage=<?php echo $intPage ?>" class="btn btn-primary btn-bitbucket" data-toggle="tooltip" data-placement="bottom" title="Editar"><i class="fa fa-pencil"></i></a>
														<a href="javascript:;" onclick="delRegistro('<?php echo $arrContenido["sl_id"]?>','','slides',<?php echo $intPage ?>);" class="btn btn-primary btn-bitbucket" data-toggle="tooltip" data-placement="bottom" title="Borrar"><i class="fa fa-trash-o"></i></a>
													</td>
												</tr>
												<?php $intCounter++; ?>
											<?php } ?>
				...
				<div class="row">
					<div class="col-lg-12 paginador">
						<?php echo printPaginado("lstSlides.php", $intQtyPages, $intPage, "sl_id");?>
					</div>
				</div>
			</div>
			<div class="footer">
