...
switch ($strOperacion) {
    case 'I':
        ...
        $fecha = invertFechaEp($_POST["fecha"]);
        $hora = armarhora($_POST["hora"]);
        $fechahora = $fecha."".$hora;

        $arrData = [
            ['value'=> $_POST["nombre"],'tipo'=> 'AN'],
            ['value'=> $_POST["disertante"],'tipo'=> 'AN'],
            ['value'=> $_POST["espacio"],'tipo'=> 'NU'],
            ['value'=> $_POST["info"],'tipo'=> 'TH'],
            ['value'=> $fechahora,'tipo'=> 'AN'],
            ['value'=> $_POST["hash"],'tipo'=> 'AN'],
            ['value'=> $_POST["publicada"],'tipo'=> 'NU'],
            ['value'=> $_POST["home"],'tipo'=> 'NU'],
            ['value'=> $_POST["mfecha"],'tipo'=> 'NU']
          ];
          
          $query = "INSERT INTO elpunto_actividades (act_titulo,act_disertante,act_espacio,act_info,act_fechahora,act_hash,act_publicado,act_home,act_vis_fecha) VALUES (?,?,?,?,?,?,?,?,?)";
        ...
        if ($_POST["imageNewRect"] != "nd") {
            ...
            if ($intQtyRecords > 0) {
                ...
                //Mueve el archivo
                rename("../assets/post-temp/" . $imagenRect . "", "../assets/actividades/" . $imagenRect . "");
            }
        }
    ...
    case 'U':
        ...
        $query = "UPDATE elpunto_actividades SET act_titulo = ?,act_disertante = ?,act_espacio = ?,act_info = ?,act_fechahora = ?,act_hash = ?,act_publicado = ?,act_home = ?,act_vis_fecha = ? WHERE act_id = ?";
        ...
        if ($_POST["iRectStat"] != 0) {
            if ($_POST["iRectStat"] == 1) {
                ...
                if ($intQtyRecords > 0) {
                    ...
                    $target_path = _CONST_PATH_IMG_ACTIVIDADES_;
    ...
    case 'D':
        ...
        $arrCont = $rsCont->fetch(PDO::FETCH_BOTH);
        //
        $arrData2 = [['value'=> $idPost,'tipo'=> 'NU'],['value'=> 2,'tipo'=> 'NU']];
        ...
        //
        $queryd = "DELETE FROM elpunto_actividades WHERE act_id = ?";
...
//
header("Location: lstActividades.php?seccion=actividades");
