...
$query = "SELECT * FROM elpunto_galerias WHERE gal_id = ?";
$rsCont = $objContenido->getOneContenido($link, $arrData, $query);
$arrCont = $rsCont->fetch(PDO::FETCH_BOTH);

//Imagenes
$arrData = [['value'=> $intIdCont,'tipo'=> 'NU']];
$queryImag = "SELECT * FROM elpunto_galeriasximag WHERE  gxi_id_gal = ? ORDER BY gxi_posicion ASC, gxi_id ASC";
$rsImag = $objContenido->getOneContenido($link,$arrData,$queryImag);

$queryImag2 = "SELECT * FROM elpunto_galeriasximag WHERE  gxi_id_gal = ? ORDER BY gxi_posicion ASC, gxi_id ASC";
