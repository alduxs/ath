    ...
    if ($intQtyPages > 1) {
        ...
        for ($i = 1; $i <= $intQtyPages; $i++) {
            if ($intPage == $i)
                ...
                $strHTML .= "<li><a href=\"" . $pstrPagina . "?intPage=" . $i . "&seccion=" . $strListado . "\">" . $i . "</a></li>";
        }
        $strHTML .= "</ul>";
    }
    return $strHTML;
...
function printPaginado($pstrPagina, $intQtyPages, $intPage, $strListado) {
    ...
    if ($intQtyPages > 1) {
      ...
      for ($i=1;$i<=$intQtyPages;$i++) {
        ...
        //si el indice no corresponde con la pagina mostrada actualmente, coloco el enlace para ir a esa pagina
        $strHTML.= "<li><a href=\"".$pstrPagina."?intPage=" . $i . "&seccion=".$strListado."\" class=\"btn btn-white btn-xs btn-bitbucket\">" . $i . "</a></li>";
      }
      $strHTML.= "</ul>";
    }
    return $strHTML;
...
// CATEGORIAS
function espaciosweb($number)
{
    switch ($number) {
        case '1':
            $espacioweb = "Espacio 1 Home";
            break;
        case '2':
            $espacioweb = "Espacio 2 Home";
            break;
        case '3':
            $espacioweb = "Actividades";
            break;
    }
    return $espacioweb;
}
